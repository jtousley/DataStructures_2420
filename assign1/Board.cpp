...
//Returns a board equal to Board b
...
string Board::move(int m,int id){
	stringstream ss;
	ss<<"State "<<id<<" to "<<"=>";
	int sub = m/ 4;
	switch (m%4)
	{
	case 0:  rotateNorth(sub); ss << "North Col" << sub; //history+="=>North Col\n"; 
	break;
	case 1:  rotateSouth(sub); ss << "South Col" << sub; //history+="=>South Col\n"; 
	break;
	case 2:  rotateEast(sub); ss << "East Row" << sub; //history+="=>East Row\n"; 
	break;
	case 3:  rotateWest(sub); ss << "West Row" << sub; //history+="=>West Row\n"; 
	break;
	}
